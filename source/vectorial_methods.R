...
pls_alg <- function(X,setX=NULL,setY=NULL) {
  ...
  if(is.null(setX) | is.null(setY)){
    N <- nrow(X) ; m <- ncol(X)
    PLS_vecs <- c() ; PLS_scores <- c(); PLS_w <- c()
    aux <- diag(m)
    C <- t(X) ; tY <- C[,2:N]/sqrt(N)  ; tX <- C[,1:(N-1)]/sqrt(N)
  }
  else{
    tX <- t(setX)/sqrt(nrow(setX)) ; tY <- t(setY)/sqrt(nrow(setY))
    N <- nrow(setX) ; m <- ncol(setX)
    PLS_vecs <- c() ; PLS_scores <- c() ; PLS_w <- c()
    aux <- diag(m)
  }
  for (i in 1:m) {
    ...
    tX <- t(P)%*%tX
    PLS_vecs <- cbind(PLS_vecs, w_new) ;  PLS_scores <- cbind(PLS_scores,t)
    PLS_w <- cbind(PLS_w,w)
  }
  colnames(PLS_vecs)<- NULL ; colnames(PLS_scores) <- NULL
...
basis_stable <- function(X, method = "pls", test = "kpss", alpha = 0.05, crit = "SC(n)", pct = "5pct", ec_det = "none") {
  if (method == "pls") {
    # pls procedure
    basis <- pls_alg(X)
  ...
  if (method == "johansen") {
    ...
    basis_S <- as.matrix(basis[,1:r_joha]) ; basis_N <- NULL
